...
\begin{figure}[p]
    \centering
    \includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{uml_classes.png}
    \caption{Diagram klas UML pakietu obliczeń rozmytych i generowania
    podsumowań. W każdej klasie wyróżniono sekcję pól (atrybutów) -- nad linią --
    oraz kluczowych operacji (metod); zagnieżdżone typy wyliczeniowe
    (np.~\texttt{Universe.Kind}, \texttt{Quantifier.Type},
    \texttt{MultiSubjectSummary.Form}) pokazano jako odrębne elementy powiązane
    z~klasą macierzystą. Pakiety: \texttt{fuzzy.set} (zbiory i przestrzeń
    rozważań), \texttt{fuzzy.mf} (funkcje przynależności -- delegacja do
    \texttt{jFuzzyLogic}), \texttt{fuzzy.linguistic} (zmienna lingwistyczna,
    etykieta, kwantyfikator), \texttt{summary} (sumaryzator/kwalifikator,
    podsumowania jednopodmiotowe), \texttt{summary.quality} (miary $T_1$--$T_{11}$),
    \texttt{summary.multi} (podsumowania wielopodmiotowe, formy I--IV) oraz
    \texttt{data} (rekordy i dostęp do bazy). Linie ciągłe oznaczają złożenie
    (pole), przerywane -- zależność użycia; trójkąt -- realizację interfejsu.}
    \label{fig:uml}
\end{figure}
\end{landscape}
\restoregeometry
...
Poniżej zwięzły opis klas, ich pól oraz kluczowych metod (rys.~\ref{fig:uml}).
